...
% Appel de la fonction Demonstrateur666f
figure; % Créer une nouvelle figure
hold on; % Permet de maintenir les tracés dans la figure
curveHandle = Demonstrateur666f(Nbre, Lag, Dose, color);
% Ajout d'une légende
legend(curveHandle, ...
...
figure;
% Paramètres fixes
Lag = 6; % En mois, délai entre les fractions/runs (supposé constant)
Dose = 3.7; % En Gbq, Activité par run d'irathérapie
...
for i = 1:Nbre
    ...
    curveHandles(i) = Demonstrateur666f(i, Lag, Dose, Colors(i, :));
end
% Légende automatique
...
for i = 1:8 % En mois, délai entre les fractions/runs (supposé constant)
    % Passer la couleur correspondant à Lag `i` à Demonstrateur666f
    curveHandles(i) = Demonstrateur666f(Nbre, i, Dose, LagColors(i, :));
end
...
for i = 1:8
    ...
    curveHandles(i) = Demonstrateur666f(Nbre, Lag, Dose, Colors(i, :));
end
% Ajout de la légende
